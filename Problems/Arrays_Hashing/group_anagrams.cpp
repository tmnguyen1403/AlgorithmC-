...
public:
    std::vector<std::vector<std::string>> groupAnagrams(std::vector<std::string>& strs) {
        ...
        /*Time complexity: O(n*klogk) 
            n: the number of elements in the strs
            k: the length of the longest string 
        */
        for (const auto& str: strs) {
            std::string sorting_str = str;
            std::sort(sorting_str.begin(), sorting_str.end());
            sorted_strs.push_back(sorting_str);
        }
        ...
        for (const auto& str: sorted_strs) {
            ...
            if (it == group_map.end() ) {
                group_map.insert(std::pair<std::string,std::vector<std::string>>(str, std::vector<std::string>()));
            }
            //add unsroted string of strs into their own anagram group
            group_map[str].push_back(strs[index]);
            ++index;
        }

...
//Approach: Using solution from blind75
//Time complexity: O(m*n*26) for the constraints of using 26 lowercase English alphabet   
//
class Solution {
public:
    std::vector<std::vector<std::string>> groupAnagrams(std::vector<std::string>& strs) {
        ...
        std::map<std::array<int, 26>, std::vector<std::string>> group_map;
        std::vector<std::vector<std::string>> group_vector_result;
        ...
        for (const auto& str : strs) {
            ...
            if  (group_map.find(count) == group_map.end()) {
             group_map.insert(std::pair<std::array<int, 26>, std::vector<std::string>>(count, std::vector<std::string>()));
            }
            group_map[count].push_back(str);
        }

        
        //save the anagram groups into group vector
        for (const auto& [key, group]: group_map) {
            group_vector_result.push_back(group);
        }

        return group_vector_result;
    }
};
